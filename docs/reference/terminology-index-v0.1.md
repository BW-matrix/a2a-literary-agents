...
| `ScenePacket` | packet type | committed system-level packet for downstream projection, rendering, and memory | use for committed scene material before per-agent projection | complete packet is not default narrator input | `scene-packet-schema-v0.1.md` |
...
| `FieldProjection` | projection policy | field-level allowlist and redaction rule for recipient views | use when splitting mixed objects such as `DialogueWindow` or `ScenePacket` | message-level visibility alone is not enough | `agent-context-packet-and-field-visibility-v0.1.md` |
...
| `MemoryDelta` | memory unit | owner-scoped update unit for `private_memory` | use for owner-specific memory entries derived from committed scenes or legal recollection paths | not objective world truth | `memory-delta-format-v0.1.md` |
...
| `Emergent Canon` | canon provenance | newly accepted canon introduced during creation | use for origin/provenance after review, then assign visibility as public, latent, or scoped | do not treat as a third visibility layer or ad hoc patching | `canon-mutation-review-checklist-v0.1.md` |
...
3. clean up canon terminology so `Emergent Canon` is treated as provenance rather than a visibility layer
